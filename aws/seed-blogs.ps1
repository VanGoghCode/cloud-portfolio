# Seed Initial Blog Posts to DynamoDB (PowerShell)

Write-Host "📝 Seeding blog posts to DynamoDB..." -ForegroundColor Green
Write-Host ""
...
# Blog Post 1
Write-Host "Adding: Designing Cloud-Native Frontends..." -ForegroundColor Yellow
$item1 = @'
{
...
aws dynamodb put-item --table-name portfolio-blog-posts --item $item1 --region $AWS_REGION --no-cli-pager
Write-Host "✅ Blog post 1 added" -ForegroundColor Green

# Blog Post 2
Write-Host "Adding: Next.js + Edge: A Practical Guide..." -ForegroundColor Yellow
$item2 = @'
{
...
aws dynamodb put-item --table-name portfolio-blog-posts --item $item2 --region $AWS_REGION --no-cli-pager
Write-Host "✅ Blog post 2 added" -ForegroundColor Green

# Blog Post 3
Write-Host "Adding: Minimalist UI: Doing More with Less..." -ForegroundColor Yellow
$item3 = @'
{
...
aws dynamodb put-item --table-name portfolio-blog-posts --item $item3 --region $AWS_REGION --no-cli-pager
Write-Host "✅ Blog post 3 added" -ForegroundColor Green

Write-Host ""
Write-Host "🎉 Successfully seeded 3 blog posts!" -ForegroundColor Green
Write-Host ""
Write-Host "To view them:"
Write-Host "  aws dynamodb scan --table-name portfolio-blog-posts --region $AWS_REGION"
